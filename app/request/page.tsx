...
export default function RequestToBook() {
  ...
  const [numGuests, setNumGuests] = useState(4); // default to 4 guests
  ...
  const handleStartDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newStart) {
      ...
      // Auto-set checkout to the day after check-in if it's empty or invalid
  ...
  const validateForm = () => {
    ...
    // Name and email are handled by HTML required + browser, but we can add more here later
  ...
  return (
    <div className="max-w-2xl mx-auto py-12 px-6">
      ...
      <form onSubmit={handleSubmit} className="relative space-y-6 bg-white p-8 rounded-2xl border">
        ...
        <a 
          href="/" 
          className="absolute top-4 right-4 text-slate-700 hover:text-slate-900 text-3xl leading-none p-1"
